...
export default function CriativoEditor() {
  ...
  function saveAll() {
    ...
    try { roteiro = roteiroTxt.trim() ? JSON.parse(roteiroTxt) : null; } catch { /* mantém antigo */ }
    save.mutate({
      id, copy, briefing,
  ...
  if (cQuery.isLoading || !c) {
    return <AppLayout title="Carregando criativo…"><div className="flex justify-center py-20"><Loader2 className="w-7 h-7 animate-spin text-primary" /></div></AppLayout>;
  }

  const angChip = angulo === "dor" ? "bg-red-50 text-red-600 border-red-200"
    : angulo === "transformacao" ? "bg-violet-50 text-violet-600 border-violet-200"
    ...
    <AppLayout
      ...
      actions={
        <div className="flex gap-2">
          ...
          <button onClick={() => sendApproval.mutate({ creativeIds: [c.id] })} disabled={sendApproval.isPending}
            className="text-xs font-black text-white px-4 py-2 rounded-lg flex items-center gap-2 disabled:opacity-50" style={{ background: "linear-gradient(180deg,#18b85c,#0f8d44)" }}>
            {sendApproval.isPending ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Send className="w-3.5 h-3.5" />}
            Enviar p/ aprovação
          </button>
        </div>
      }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.05fr] gap-5">
          ...
          <div className="bg-white rounded-xl border border-[#e6ebf3] shadow-sm overflow-hidden">
            <div className="aspect-square bg-[#f6f8fc] relative">
              {c.imageUrl
                ...
                : <div className="absolute inset-0 flex items-center justify-center"><ImageIcon className="w-12 h-12 text-[#cfd8e6]" /></div>}
              {regen.isPending && (
                <div className="absolute inset-0 bg-black/60 backdrop-blur-sm flex flex-col items-center justify-center gap-2">
                  <Loader2 className="w-10 h-10 text-white animate-spin" />
                  <p className="text-sm text-white font-bold">Regerando imagem…</p>
              ...
              {setImg.isPending && (
                <div className="absolute inset-0 bg-black/60 flex flex-col items-center justify-center gap-2">
                  <Loader2 className="w-10 h-10 text-white animate-spin" />
                  <p className="text-sm text-white font-bold">Enviando imagem…</p>
                </div>
              )}
            </div>
            {/* Ações sobre a imagem */}
            <div className="p-4 grid grid-cols-2 gap-2 border-t border-[#e6ebf3]">
              <button onClick={handleRegen} disabled={regen.isPending}
                className="text-xs font-black text-white px-3 py-2.5 rounded-lg flex items-center justify-center gap-2 disabled:opacity-50" style={{ background: "linear-gradient(180deg,#ff421f,#f0200d)" }}>
                {regen.isPending ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Sparkles className="w-3.5 h-3.5" />}
                Regerar imagem
              </button>
              <button onClick={() => fileInput.current?.click()} disabled={setImg.isPending}
              ...
              <input ref={fileInput} type="file" accept="image/*" className="hidden" onChange={e => handleUpload(e.target.files?.[0])} />

              {c.imageUrl && (
                <a href={c.imageUrl} download className="text-xs font-bold text-[#61708a] hover:text-[#070b17] border border-[#e6ebf3] px-3 py-2.5 rounded-lg flex items-center justify-center gap-2 col-span-1">
                  <Download className="w-3.5 h-3.5" /> Baixar
                </a>
              )}
              <button onClick={() => window.open(c.imageUrl, "_blank")} className="text-xs font-bold text-[#61708a] hover:text-[#070b17] border border-[#e6ebf3] px-3 py-2.5 rounded-lg flex items-center justify-center gap-2 col-span-1">
                <Eye className="w-3.5 h-3.5" /> Abrir em tela cheia
              </button>
            </div>
            <p className="px-4 pb-3 text-[10px] text-[#61708a]">Regerar usa o prompt da aba "Direção de Arte". 15 créditos por geração.</p>
          </div>
          ...
          {versions.length > 0 && (
            <div className="bg-white rounded-xl border border-[#e6ebf3] shadow-sm p-4">
              <h3 className="text-xs font-black text-[#070b17] uppercase tracking-wide mb-2 flex items-center gap-1.5">
                <History className="w-3.5 h-3.5 text-[#ff3217]" /> Histórico ({versions.length})
              </h3>
              <div className="grid grid-cols-4 gap-2">
                {versions.slice(0, 8).map((v, i) => (
                  <button key={i} onClick={() => revert.mutate({ id: c.id, imageUrl: v.imageUrl })} disabled={revert.isPending}
                    ...
                    <span className="absolute bottom-0.5 left-0.5 text-[8px] font-black text-white bg-black/60 px-1 rounded uppercase">{v.kind ?? "v"}</span>
                  </button>
                ))}
              </div>
              <p className="text-[10px] text-[#61708a] mt-2">Clique numa versão para restaurar.</p>
            </div>
          )}
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {pilar && <span className="text-[10px] font-black text-[#ff3217] bg-[#fff1ef] border border-[#ffd0c8] rounded-full px-2.5 py-1">{pilar}</span>}
            {formato && <span className="text-[10px] font-black text-[#61708a] bg-[#f1f4f9] border border-[#e6ebf3] rounded-full px-2.5 py-1 uppercase flex items-center gap-1">{formato === "reels" && <Film className="w-2.5 h-2.5" />}{formato}</span>}
            {angulo && <span className={`text-[10px] font-black border rounded-full px-2.5 py-1 ${angChip}`}>{angulo}</span>}
            <span className="text-[10px] font-bold text-[#9aa7bd] ml-auto">ID #{c.id} · {c.status}</span>
          </div>

          {/* Tabs */}
          <div className="bg-white rounded-xl border border-[#e6ebf3] shadow-sm">
            <div className="flex border-b border-[#e6ebf3]">
              <TabBtn active={tab === "brief"} onClick={() => setTab("brief")} icon={<FileText className="w-3.5 h-3.5" />}>Brief</TabBtn>
              <TabBtn active={tab === "art"} onClick={() => setTab("art")} icon={<Palette className="w-3.5 h-3.5" />}>Direção de Arte</TabBtn>
              <TabBtn active={tab === "image"} onClick={() => setTab("image")} icon={<ImageIcon className="w-3.5 h-3.5" />}>Imagem</TabBtn>
            </div>
            ...
            <div className="p-5 space-y-4">
              ...
              {tab === "brief" && (
                <>
                  ...
                  <Field icon={<Megaphone className="w-3 h-3" />} label="Legenda / Copy" hint="Texto do post. Tom alinhado ao DNA da marca.">
                    <textarea value={copy} onChange={e => { setCopy(e.target.value); setDirty(true); }} rows={5}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <Field icon={<Hash className="w-3 h-3" />} label="Hashtags" hint="Separadas por espaço.">
                      <input value={hashtagsTxt} onChange={e => { setHashtagsTxt(e.target.value); setDirty(true); }}
                        placeholder="#financas #investimentos"
                        className="w-full border border-[#e6ebf3] rounded-lg px-3 py-2.5 text-sm bg-[#f6f8fc] focus:outline-none focus:border-[#ff3217]" />
                    </Field>
                    <Field icon={<Tag className="w-3 h-3" />} label="CTA" hint="Chamada de ação.">
                      <input value={cta} onChange={e => { setCta(e.target.value); setDirty(true); }}
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    ...
                    <Field label="Formato">
                      <select value={formato} onChange={e => { setFormato(e.target.value); setDirty(true); }}
                        className="w-full border border-[#e6ebf3] rounded-lg px-3 py-2.5 text-sm bg-[#f6f8fc] focus:outline-none focus:border-[#ff3217]">
                  ...
                  {(formato === "reels" || formato === "carrossel" || roteiroTxt) && (
                    <Field icon={<Film className="w-3 h-3" />} label="Roteiro (JSON)" hint='Estrutura: { "gancho3s": "...", "cenas": [{ "tempo": "0-3s", "acao": "...", "audio": "..." }], "cta": "..." }'>
              ...
              {tab === "art" && (
                <>
                  <Field icon={<Palette className="w-3 h-3" />} label="Prompt de imagem (visualPrompt)" hint="Briefing de direção de arte em inglês. Cena, luz, lente, paleta da marca. Este prompt é usado pelo botão 'Regerar'.">
                    <textarea value={visualPrompt} onChange={e => { setVisualPrompt(e.target.value); setDirty(true); }} rows={8}
                  ...
                  <div className="rounded-lg border border-[#fff1ef] bg-[#fff8f6] p-4 space-y-3">
                    ...
                    <Field label="Override do prompt (opcional)" hint="Se preencher, é este prompt que vai pro modelo (em vez do salvo acima).">
                      <textarea value={promptOverride} onChange={e => setPromptOverride(e.target.value)} rows={3}
                    ...
                    <label className="flex items-center gap-2 text-xs font-bold text-[#22304b] cursor-pointer">
                      <input type="checkbox" checked={keepStyle} onChange={e => setKeepStyle(e.target.checked)} />
                      Manter estilo da marca como referência (img2img)
                    </label>
                    <button onClick={handleRegen} disabled={regen.isPending}
                      className="w-full text-sm font-black text-white px-4 py-3 rounded-lg flex items-center justify-center gap-2 disabled:opacity-50" style={{ background: "linear-gradient(180deg,#ff421f,#f0200d)" }}>
                      {regen.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <Sparkles className="w-4 h-4" />}
                      {regen.isPending ? "Gerando…" : "Regerar imagem (15 créditos)"}
                    </button>
                  </div>

                  {meta.clonedFrom && (
                    <div className="rounded-lg border border-[#e6ebf3] p-3 flex items-center gap-3">
                      <img src={meta.clonedFrom} alt="" className="w-12 h-12 rounded object-cover" />
                      <div className="flex-1">
                        <p className="text-[10px] font-black text-[#61708a] uppercase">Referência de estilo</p>
                        <p className="text-[11px] text-[#22304b] font-semibold">Clonado a partir desta imagem.</p>
                      </div>
                    </div>
                  )}
                </>
              ...
              {tab === "image" && (
                <>
                  <div className="rounded-lg border-2 border-dashed border-[#e6ebf3] hover:border-[#ff3217] p-8 text-center cursor-pointer transition-colors"
                    onClick={() => fileInput.current?.click()}
                    onDragOver={e => e.preventDefault()}
                    onDrop={e => { e.preventDefault(); handleUpload(e.dataTransfer.files?.[0]); }}>
                    <Upload className="w-8 h-8 text-[#cfd8e6] mx-auto mb-2" />
                    <p className="text-sm font-bold text-[#22304b]">Subir imagem própria</p>
                    <p className="text-[11px] text-[#61708a] mt-1">JPG, PNG ou WebP. Clique ou arraste aqui. A imagem atual vai pro histórico — você pode restaurar depois.</p>
                  </div>

                  <div className="rounded-lg border border-[#e6ebf3] p-3 space-y-2">
                    <h4 className="text-xs font-black text-[#070b17] uppercase tracking-wide">Detalhes técnicos</h4>
                    <Row label="Modelo" value={meta.model ?? "—"} />
                    <Row label="Proporção" value={c.ratio ?? "—"} />
                    <Row label="Gerada em" value={c.createdAt ? new Date(c.createdAt).toLocaleString("pt-BR") : "—"} />
                    {c.imageUrl && <Row label="URL" value={<a className="text-[#ff3217] hover:underline truncate block" href={c.imageUrl} target="_blank" rel="noreferrer">{c.imageUrl}</a>} />}
                  </div>
                </>
              )}
            </div>
          ...
          <div className="bg-white rounded-xl border border-[#e6ebf3] shadow-sm p-4 flex items-center justify-between">
            <p className="text-[11px] text-[#61708a]">{hasUnsavedChanges ? "Você tem alterações não salvas." : "Todas as alterações salvas."}</p>
            <div className="flex gap-2">
              <button onClick={saveAll} disabled={!hasUnsavedChanges || save.isPending}
                className="text-xs font-black text-white px-4 py-2 rounded-lg flex items-center gap-2 disabled:opacity-40" style={{ background: "linear-gradient(180deg,#071b44,#0d2a5e)" }}>
                {save.isPending ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Save className="w-3.5 h-3.5" />}
                Salvar alterações
              </button>
            </div>
      ...
      {confirmDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4" onClick={() => setConfirmDelete(false)}>
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6" onClick={e => e.stopPropagation()}>
            ...
            <h2 className="text-lg font-black text-[#070b17]">Excluir este criativo?</h2>
            <p className="text-sm text-[#22304b] mt-2 leading-relaxed">
              Você está prestes a excluir <span className="font-black">{c.briefing}</span>.
              A imagem, o brief e todas as versões anteriores vão sumir.
            </p>
            <p className="text-[11px] text-[#61708a] mt-2">Esta ação não pode ser desfeita.</p>
            <div className="grid grid-cols-2 gap-2 mt-4">
              <button onClick={() => setConfirmDelete(false)} className="text-xs font-bold text-[#070b17] border border-[#e6ebf3] hover:bg-[#f6f8fc] px-4 py-2.5 rounded-lg">
                Cancelar
              </button>
              <button onClick={() => deleteCreative.mutate({ id: c.id })} disabled={deleteCreative.isPending}
                ...
                {deleteCreative.isPending ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Trash2 className="w-3.5 h-3.5" />}
...
function TabBtn({ active, onClick, icon, children }: { active: boolean; onClick: () => void; icon: React.ReactNode; children: React.ReactNode }) {
  return (
    <button onClick={onClick} className={`flex-1 text-xs font-black py-3 px-4 flex items-center justify-center gap-1.5 transition-colors ${active ? "text-[#ff3217] border-b-2 border-[#ff3217] bg-[#fff8f6]" : "text-[#61708a] hover:text-[#070b17] border-b-2 border-transparent"}`}>
      {icon}{children}
    </button>
  );
}
...
function Row({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    <div className="flex items-center justify-between gap-3 text-[11px]">
      <span className="font-black text-[#61708a] uppercase tracking-wide">{label}</span>
      <span className="text-[#22304b] font-semibold text-right truncate max-w-[60%]">{value}</span>
    </div>
  );
}
