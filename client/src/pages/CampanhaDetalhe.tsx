...
export default function CampanhaDetalhe() {
  ...
  const creatives = trpc.creatives.list.useQuery(
    { campaignId: id },
    { enabled: Number.isFinite(id) }
  );
  const metrics = trpc.metrics.byCampaign.useQuery(
    { campaignId: id },
    { enabled: Number.isFinite(id) }
  );
  const logs = trpc.dispatch.logs.useQuery(
    { campaignId: id },
    { enabled: Number.isFinite(id) }
  );
  const calibration = trpc.calibration.byCampaign.useQuery(
    { campaignId: id },
    { enabled: Number.isFinite(id) }
  );
  ...
  const budgetProgress = budgetTotal
  ...
  const packageText = campaign
    ? [
        ...
        ...(creatives.data ?? []).map(
          (creative: any) =>
            `- #${creative.id}: ${(creative.copy || creative.briefing || "").slice(0, 150)}`
        ),
      ].join("\n")
    : "";
  ...
  if (!campaign) {
    return (
      <AppLayout
        ...
        subtitle="Volte para a lista e escolha uma campanha ativa."
        journeyActive="campanhas"
      >
        <div className="rounded-2xl border border-[#e6ebf3] bg-white p-10 text-center shadow-sm">
          <p className="text-base font-black text-[#071b44]">
            Essa campanha nao existe neste perfil.
          </p>
          <Link href="/campanhas">
            <a className="btn-action-primary mt-5 inline-flex px-5 py-3 text-sm">
      ...
      actions={
        <div className="flex gap-2 flex-wrap justify-end">
          ...
          {campaign.status === "ativa" ? (
            <button
              onClick={() => update.mutate({ id, status: "pausada" })}
            ...
            <button
              onClick={() => update.mutate({ id, status: "ativa" })}
              disabled={update.isPending}
              className="btn-action-primary px-4 py-2.5 text-sm flex items-center gap-2 disabled:opacity-50"
            >
    ...
    >
      <section className="grid grid-cols-1 xl:grid-cols-[1.05fr_.95fr] gap-5 mb-6">
        <div className="rounded-3xl bg-[#071b44] text-white p-6 shadow-sm">
          ...
          <p className="text-sm text-white/78 leading-relaxed mt-4">
            {campaign.targetAudience ||
              "Publico ainda nao detalhado. Use a campanha assistida do diagnostico para preencher melhor essa parte."}
          </p>
          <div className="flex flex-wrap gap-2 mt-5">
            {((campaign.channels as string[]) ?? []).map(channel => (
          ...
          <div className="rounded-2xl border border-white/10 bg-white/8 p-4 mt-5">
            ...
            <p className="text-sm font-black mt-3">
              {brl(budgetSpent)} de {brl(budgetTotal)}
            </p>
          </div>
        </div>
      ...
      <section className="grid grid-cols-1 xl:grid-cols-[1fr_360px] gap-5 mb-6">
        <div className="rounded-3xl border border-[#e6ebf3] bg-white p-6 shadow-sm">
          <div className="flex items-start justify-between gap-3 flex-wrap">
            <div>
              <h2 className="text-xl font-black text-[#071b44] flex items-center gap-2">
                <Sparkles className="w-5 h-5 text-[#ff3217]" /> Criativos
                vinculados
              </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-5">
            {(creatives.data ?? []).length ? (
              (creatives.data ?? []).map((creative: any) => (
                <article
                  key={creative.id}
                  className="rounded-2xl border border-[#e6ebf3] overflow-hidden bg-[#fbfcff]"
                >
                  {creative.imageUrl && (
                    <img
                  ...
                  <div className="p-4">
                    <p className="text-[10px] font-black text-[#ff3217] uppercase">
                      #{creative.id} - {creative.status}
                    </p>
                    <p className="text-sm font-bold text-[#22304b] leading-relaxed mt-2 line-clamp-5">
                      {creative.copy || creative.briefing}
                    </p>
                    <Link
                      href={`/criativos/${creative.id}?returnTo=${encodeURIComponent(`/campanhas/${id}`)}&closeOnSave=1`}
                    >
                      <a className="mt-3 w-full rounded-xl border border-[#e6ebf3] bg-white px-4 py-2 text-xs font-black text-[#071b44] hover:bg-[#f8fafc] inline-flex justify-center">
                        Editar no Estudio
                      </a>
                    </Link>
                  </div>
              ...
              <div className="md:col-span-2 rounded-2xl border border-dashed border-[#d8e0ec] bg-[#fbfcff] p-8 text-center">
                <p className="text-sm font-black text-[#071b44]">
                  Nenhum criativo vinculado.
                </p>
                <p className="text-xs text-[#61708a] mt-1">
                  Vincule criativos no Estudio ou envie posts pela Aprovação.
                </p>
              </div>
            )}
          </div>
        ...
        <aside className="space-y-4">
          <div className="rounded-3xl border border-[#e6ebf3] bg-white p-5 shadow-sm">
            <h3 className="text-sm font-black text-[#071b44] flex items-center gap-2">
              <RefreshCcw className="w-4 h-4 text-[#ff3217]" /> Aprendizado
            </h3>
            <p className="text-xs text-[#61708a] leading-relaxed mt-2">
              Gere uma leitura quando houver sinais suficientes para ajustar
              verba, criativo ou publico.
            </p>
            <button
              onClick={() => analyze.mutate({ campaignId: id })}
              disabled={analyze.isPending}
              className="mt-4 w-full btn-action-primary px-4 py-2.5 text-xs flex items-center justify-center gap-2 disabled:opacity-50"
            >
              {analyze.isPending ? (
            ...
            <div className="space-y-3 mt-4">
              {(calibration.data ?? []).slice(0, 3).map((item: any) => (
                ...
                >
                  ...
                  <p className="text-xs font-bold text-[#22304b] leading-relaxed mt-1 line-clamp-4">
                    {item.analysis}
                  </p>
                </div>
              ))}
          ...
          <div className="rounded-3xl border border-[#e6ebf3] bg-white p-5 shadow-sm">
            <h3 className="text-sm font-black text-[#071b44] flex items-center gap-2">
              <Send className="w-4 h-4 text-[#ff3217]" /> Disparos
            </h3>
      ...
      <section className="rounded-3xl border border-[#e6ebf3] bg-white p-6 shadow-sm">
        <h2 className="text-xl font-black text-[#071b44] flex items-center gap-2">
          <Calendar className="w-5 h-5 text-[#ff3217]" /> Janela de campanha
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
          ...
          <Small
            ...
            value={
              totals.conversions
                ? "Com conversao registrada"
                : totals.clicks
                  ? "Com clique, sem conversao"
                  : "Aguardando volume"
            }
          />
        </div>
