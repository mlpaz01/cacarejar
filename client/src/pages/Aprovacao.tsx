...
export default function Aprovacao() {
  ...
  useEffect(() => {
    ...
    setBudgetByExp(prev => {
      const next = { ...prev };
      for (const exp of pending.data as any[]) {
        if (next[exp.id] === undefined)
          next[exp.id] = exp.budgetDailyCents || 3000;
      }
      return next;
    });
  }, [pending.data]);
    ...
    >
      {pending.isLoading ? (
        ...
        pending.data.map((exp: any) => {
          const selectedIds = exp.variants
            .filter((v: any) => sel[v.id])
            .map((v: any) => v.id);
          ...
          const channels = Array.from(
            new Set(
              exp.variants.map((v: any) =>
                channelLabel(v.creative?.channels, exp.channel)
              )
            )
          );
          const origins = Array.from(
            new Set(exp.variants.map((v: any) => originLabel(v.creative)))
          );
          const perPostDailyCents = selectedIds.length
            ? Math.floor(budgetDailyCents / selectedIds.length)
            : 0;
          const approvalPackageText = [
            ...
            ...selectedVariants.flatMap((v: any, index: number) => {
              ...
              return [
                ...
                c.lente || c.pilar || c.formato
                  ? `DNA: ${[c.lente, c.pilar, c.formato].filter(Boolean).join(" | ")}`
                  : "",
                "",
              ];
            }),
          ...
          return (
            <div
              key={exp.id}
              className="bg-white rounded-xl border border-[#e6ebf3] p-5 shadow-sm mb-6"
            >
              <div className="flex items-start justify-between gap-4 flex-wrap mb-4">
                <div>
                  <h3 className="text-base font-black text-[#070b17]">
                    {exp.name}
                  </h3>
                  <p className="text-xs text-[#61708a] mt-1">
                    {selectedIds.length} de {exp.variants.length} posts
                    selecionados para revisao final.
                  </p>
                  <div className="flex flex-wrap gap-2 mt-3">
                    <span className="inline-flex items-center gap-1 rounded-full bg-[#f6f8fc] border border-[#e6ebf3] px-3 py-1 text-[10px] font-black text-[#071b44]">
                      <Layers3 className="w-3 h-3" /> {channels.join(" + ")}
                    </span>
                    <span className="inline-flex items-center gap-1 rounded-full bg-[#fff1ef] border border-[#ffd0c8] px-3 py-1 text-[10px] font-black text-[#ff3217]">
                      <Send className="w-3 h-3" /> {origins.join(" + ")}
                    </span>
                  </div>
                </div>
                ...
                <div>
                  <div className="flex items-center justify-between gap-2 mb-3">
                    <p className="text-xs font-black text-[#071b44]">
                      Posts que entram no teste
                    </p>
                    <button
                      onClick={() => {
                        const allSelected =
                          selectedIds.length === exp.variants.length;
                        setSel(s => {
                          const next = { ...s };
                          for (const v of exp.variants)
                    ...
                    {exp.variants.map((v: any) => {
                      ...
                      return (
                        <div
                          ...
                          style={{
                            borderColor: on ? "#18b85c" : "#e6ebf3",
                            boxShadow: on
                              ? "0 6px 18px rgba(24,184,92,.13)"
                              : "none",
                          }}
                        >
                          <div className="relative">
                            <img
                              src={c.imageUrl}
                              alt=""
                              className="w-full aspect-[16/10] object-cover bg-[#f6f8fc]"
                            />
                            <span
                              ...
                              style={{
                                background: on
                                  ...
                                  : "rgba(255,255,255,.92)",
                                color: on ? "#fff" : "#61708a",
                                border: on ? "none" : "1px solid #e6ebf3",
                              }}
                            >
                              {on && <Check className="w-3 h-3" />}{" "}
                              {on ? "Aprovado" : "Nao usar"}
                            </span>
                          </div>
                          <div className="p-3 bg-white">
                            <div className="flex flex-wrap gap-1.5 mb-2">
                              <span className="text-[9px] font-black text-[#071b44] bg-[#eef4ff] border border-[#dbe8ff] rounded px-1.5 py-0.5">
                                {channelLabel(c.channels, exp.channel)}
                              </span>
                              <span className="text-[9px] font-black text-[#ff3217] bg-[#fff1ef] border border-[#ffd0c8] rounded px-1.5 py-0.5">
                                {originLabel(c)}
                              </span>
                            </div>
                            <p className="text-xs text-[#22304b] font-semibold leading-snug min-h-[116px] line-clamp-8">
                              {c.copy}
                            </p>
                            ...
                            {c.generationMeta?.humanReview && (
                              <div className="mt-3 rounded-lg border border-[#e6ebf3] bg-[#fbfcff] p-2">
                                ...
                                <p className="text-xs font-bold text-[#071b44] mt-1">
                                  {c.generationMeta.humanReview.score
                                    ? `${c.generationMeta.humanReview.score}/100`
                                    : "Sem nota"}
                                  {c.generationMeta.humanReview.note
                                    ? ` - ${c.generationMeta.humanReview.note}`
                                    : ""}
                                </p>
                              </div>
                            ...
                            {c.id && (
                              <button
                                ...
                                onClick={() =>
                                  navigate(
                                    `/criativos/${c.id}?returnTo=${encodeURIComponent(`/aprovacao?studioReturn=${Date.now()}`)}&closeOnSave=1`
                                  )
                                }
                ...
                <aside className="rounded-xl border border-[#e6ebf3] bg-[#fbfcff] p-4 h-fit">
                    ...
                    {BUDGETS.map(b => {
                      ...
                      return (
                        <button
                          ...
                          style={{
                            borderColor: active ? "#ff3217" : "#e6ebf3",
                            background: active ? "#fff1ef" : "#fff",
                          }}
                        >
                          <span className="text-xs font-black text-[#071b44]">
                            {b.label}
                          </span>
                          <span className="block text-[11px] font-bold text-[#61708a]">
                            {b.note}
                          </span>
                        </button>
                      );
                  ...
                  <div className="mt-4 rounded-lg bg-white border border-[#e6ebf3] p-3">
                    ...
                    <div className="grid grid-cols-2 gap-2 mt-3">
                      <div className="rounded-lg bg-[#f6f8fc] p-2">
                        ...
                        <p className="text-lg font-black text-[#071b44]">
                          {selectedIds.length}
                        </p>
                      </div>
                      <div className="rounded-lg bg-[#f6f8fc] p-2">
                        ...
                        <p className="text-lg font-black text-[#071b44]">
                          {selectedIds.length ? brl(perPostDailyCents) : "-"}
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="mt-4 rounded-lg bg-[#071b44] text-white p-3">
                    ...
                    <p className="text-[11px] leading-relaxed mt-1 text-white/85">
                      ...
                      partes iguais de {brl(budgetDailyCents)}/dia. Depois, o
                      motor mede resultado e puxa mais verba para o post
                      vencedor, mantendo um piso de teste para os outros.
                    </p>
                  </div>
                  <div className="mt-3 grid grid-cols-3 gap-2">
                    <span className="rounded-lg bg-white border border-[#e6ebf3] px-2 py-2 text-[10px] font-black text-center text-[#071b44]">
                      CTR
                    </span>
                    <span className="rounded-lg bg-white border border-[#e6ebf3] px-2 py-2 text-[10px] font-black text-center text-[#071b44]">
                      CPL
                  ...
                  <div className="mt-3 rounded-lg bg-white border border-[#e6ebf3] p-3">
                    <p className="text-xs font-black text-[#071b44] flex items-center gap-1.5">
                      <ShieldCheck className="w-3.5 h-3.5 text-[#18b85c]" />{" "}
