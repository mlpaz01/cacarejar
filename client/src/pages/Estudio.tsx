...
import { Loader2, Sparkles, Coins, ArrowRight, Wand2, Upload, X, Copy } from "lucide-react";
...
export default function Estudio() {
  ...
  const handleRefFile = (file?: File) => {
    ...
    reader.onload = async () => {
      try {
        const r = await uploadRef.mutateAsync({ dataUrl: String(reader.result) });
        setRefUrl(r.url);
  ...
  useEffect(() => {
    ...
    if (ref) { setRefUrl(ref); sessionStorage.removeItem("clone_ref"); }
  }, []);
  ...
  const decideForMe = () => {
    ...
    if (prod.length < 2) { navigate("/diagnostico"); return; }
    if (!produto) setProduto(prod);
    if (p?.suggestedFactors) setSel(p.suggestedFactors);
    gen.mutate({ produto: prod, ratio, baseFactors: cleanSel(p?.suggestedFactors ?? sel), qty: 6, az: true, refImageUrl: refUrl || undefined });
  };
  ...
  const gen = trpc.studio.generate.useMutation({
    onSuccess: d => {
      ...
      if (d.stopped === "saldo_insuficiente") toast.error("Saldo de créditos insuficiente. Compre mais em Créditos.");
      else if (d.stopped === "cota_diaria") toast.error("Cota diária atingida. Tente amanhã ou use pacote avulso.");
      else toast.success(`${d.generated} variações geradas! 🥚`);
    },
    onError: e => toast.error(e.message || "Erro ao gerar"),
  });

  const keyFactorDefs = (factors.data ?? []).filter((f: any) => KEY_FACTORS.includes(f.key));
  ...
  return (
    <AppLayout
      ...
      actions={
        <>
        <button onClick={() => navigate("/criativos")} className="text-xs font-bold text-[#61708a] hover:text-[#070b17] flex items-center gap-1.5 px-3 py-2">
        ...
        <span className="text-xs font-bold text-[#071b44] bg-[#f6f8fc] border border-[#e6ebf3] rounded-full px-3 py-1.5 flex items-center gap-1.5">
          <Coins className="w-3.5 h-3.5 text-[#ff3217]" /> {wallet.data?.balanceCC ?? 0} CC
        </span>
        </>
      }
    >
      ...
      {plan.data ? (
        <div className="rounded-xl p-5 mb-6 flex flex-col md:flex-row md:items-center justify-between gap-3"
          style={{ background: "linear-gradient(135deg,#071b44,#0d2a5e)" }}>
          <div className="flex items-center gap-3">
            <span className="text-2xl">🎯</span>
            <div>
              <p className="text-sm font-black text-white">Deixe o Estrategista decidir por você</p>
              <p className="text-xs text-white/60">Geramos 6 variações com as características que o seu plano sugere — você não precisa configurar nada.</p>
            </div>
          </div>
          <button onClick={decideForMe} disabled={gen.isPending}
        ...
        <div className="rounded-xl border border-[#ffd0c8] bg-[#fff1ef] p-4 mb-6 flex items-center justify-between gap-3">
          <p className="text-sm font-semibold text-[#22304b]">
            💡 Faça o <b>diagnóstico</b> primeiro — o Estrategista monta seu plano e pré-preenche tudo aqui.
          </p>
          <button onClick={() => navigate("/diagnostico")} className="text-xs font-black text-white px-4 py-2 rounded-lg whitespace-nowrap" style={{ background: "linear-gradient(180deg,#ff421f,#f0200d)" }}>
      ...
      <div className="bg-white rounded-xl border border-[#e6ebf3] p-5 shadow-sm mb-6">
        <label className="text-xs font-black text-[#071b44] uppercase tracking-wide">O que você vai anunciar?</label>
        <textarea
          value={produto}
        ...
        <div className="flex flex-wrap items-center gap-2 mt-3">
          <span className="text-xs font-black text-[#071b44] uppercase tracking-wide mr-1">Proporção</span>
          {RATIOS.map(r => (
            <Chip key={r.v} on={ratio === r.v} onClick={() => setRatio(r.v)}>{r.label}</Chip>
          ))}
        </div>

        {/* Clonar post de sucesso */}
        <div className="mt-4">
          <span className="text-xs font-black text-[#071b44] uppercase tracking-wide">Clonar um post de sucesso <span className="text-[#61708a] font-semibold normal-case">(opcional)</span></span>
          {refUrl ? (
            <div className="flex items-center gap-3 mt-2 border border-[#18b85c] bg-[#f3fbf6] rounded-lg p-2.5">
              <img src={refUrl} alt="" className="w-14 h-14 rounded-md object-cover" />
              <div className="flex-1">
                <p className="text-xs font-bold text-[#0c7a3c] flex items-center gap-1.5"><Copy className="w-3.5 h-3.5" /> Vamos clonar e variar este post</p>
                <p className="text-[11px] text-[#61708a]">As variações vão manter o estilo, mudando os fatores do Teste A/Z.</p>
              </div>
              <button onClick={() => setRefUrl("")} className="p-1.5 text-[#61708a] hover:text-[#ff3217]"><X className="w-4 h-4" /></button>
            </div>
          ) : (
            <label className="flex items-center gap-2 mt-2 border border-dashed border-[#c7cdd8] rounded-lg px-4 py-3 cursor-pointer hover:border-[#ff3217] text-sm text-[#61708a] font-semibold w-fit">
              ...
              {uploading ? "Enviando…" : "Enviar imagem de um post que funcionou"}
              <input type="file" accept="image/*" className="hidden" onChange={e => handleRefFile(e.target.files?.[0])} />
            </label>
          )}
        </div>
        ...
        <div
          ...
          style={{ background: az ? "#fff1ef" : "#f6f8fc", border: `1.5px solid ${az ? "#ff3217" : "#e6ebf3"}` }}
          onClick={() => setAz(a => !a)}
        >
          <span className="text-sm font-bold" style={{ color: az ? "#ff3217" : "#61708a" }}>
            🥚 Teste A/Z — gerar 6 variações cruzando características
          </span>
          <span className="w-10 h-5 rounded-full relative transition-colors" style={{ background: az ? "#ff3217" : "#c7cdd8" }}>
            <span className="absolute top-0.5 w-4 h-4 bg-white rounded-full transition-all" style={{ left: az ? "22px" : "2px" }} />
          </span>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-3 mt-4">
          {keyFactorDefs.map((f: any) => (
            <div key={f.key}>
              <p className="text-[11px] font-bold text-[#61708a] mb-1">{f.label}</p>
              <div className="flex flex-wrap gap-1.5">
                {(f.values as any[]).filter(v => v.key !== "na").slice(0, 6).map(v => (
                  <Chip key={v.key} small on={sel[f.key] === v.key} onClick={() => pick(f.key, v.key)}>
                    {v.label}
                  </Chip>
                ))}
              </div>
        ...
        <div className="flex items-center justify-between mt-5">
          ...
          <button
            ...
            onClick={() => gen.mutate({ produto, ratio, baseFactors: cleanSel(sel), qty: az ? 6 : 1, az, refImageUrl: refUrl || undefined })}
      ...
      {results.length > 0 && (
        <>
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-black text-[#070b17]">Variações geradas</h3>
            <button
              onClick={() => sendApproval.mutate({ creativeIds: results.map(r => r.id) })}
              ...
              style={{ background: "linear-gradient(180deg,#ff421f,#f0200d)" }}
            >
              {sendApproval.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <ArrowRight className="w-4 h-4" />}
      ...
      {results.length === 0 && recent.data && recent.data.length > 0 && (
        <>
          <h3 className="text-sm font-black text-[#070b17] mb-3">Criados recentemente</h3>
...
function Grid({ items }: { items: any[] }) {
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
      {items.map(c => (
        <div key={c.id} className="bg-white rounded-xl border border-[#e6ebf3] overflow-hidden shadow-sm">
          <img src={c.imageUrl} alt="" className="w-full aspect-square object-cover bg-[#f6f8fc]" />
          <div className="p-3">
            <p className="text-xs text-[#22304b] font-semibold leading-snug min-h-[34px]">{c.copy}</p>
            <div className="flex flex-wrap gap-1 mt-2">
              {c.lente && <Tag>{c.lente}</Tag>}
              {c.formato && <Tag>{c.formato}</Tag>}
              {c.factorValues?.img_cor_predominante && <Tag>{c.factorValues.img_cor_predominante}</Tag>}
            </div>
          </div>
        </div>
      ))}
