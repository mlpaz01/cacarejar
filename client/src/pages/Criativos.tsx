...
export default function Criativos() {
  ...
  const deleteCreative = trpc.studio.deleteCreative.useMutation({
    onSuccess: () => { utils.creatives.list.invalidate(); toast.success("Criativo excluído"); },
    onError: e => toast.error(e.message || "Erro ao excluir"),
  });
  const [confirmDelete, setConfirmDelete] = useState<{ id: number; preview: string } | null>(null);
  ...
  const editQuery = trpc.studio.getCreative.useQuery({ id: editId! }, { enabled: editId != null });
  const editC = editQuery.data as any;
  const editMeta = (editC?.generationMeta ?? {}) as any;

  useEffect(() => { if (editC) setEditCopy(editC.copy ?? ""); }, [editC?.id]);
  ...
  const saveCopy = trpc.studio.updateCreative.useMutation({
    onSuccess: () => { utils.creatives.list.invalidate(); editQuery.refetch(); toast.success("Alterações salvas!"); },
    onError: e => toast.error(e.message || "Erro ao salvar"),
  });
  const regen = trpc.studio.regenerateImage.useMutation({
    onSuccess: () => { editQuery.refetch(); utils.creatives.list.invalidate(); toast.success("Nova imagem gerada!"); },
    onError: e => toast.error(e.message || "Erro ao regerar"),
  });
  const sendApproval = trpc.approvals.sendToApproval.useMutation({
    onSuccess: () => { setEditId(null); toast.success("Enviado para aprovação!"); },
    ...
    <AppLayout
      ...
      actions={
        <div className="flex gap-2">
          <button onClick={() => navigate("/estudio")} className="text-sm font-black text-white px-4 py-2.5 rounded-lg flex items-center gap-2" style={{ background: "linear-gradient(180deg,#ff421f,#f0200d)" }}>
    ...
    >
                  ...
                  <div className="p-3">
                    ...
                    <div className="grid grid-cols-[1fr_auto] gap-1.5 mb-1.5">
                      ...
                      <Button
                        ...
                        onClick={() => setConfirmDelete({ id: creative.id, preview: creative.briefing ?? "este criativo" })}
      ...
      <Dialog open={editId != null} onOpenChange={(o) => !o && setEditId(null)}>
        <DialogContent className="max-w-3xl bg-card border-border max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-sm font-semibold">Editar criativo</DialogTitle>
          </DialogHeader>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 py-1">
              ...
              <div>
                <div className="aspect-square rounded-xl overflow-hidden bg-muted border border-border relative">
                  {editC.imageUrl
                    ? <img src={editC.imageUrl} alt="" className="w-full h-full object-cover" />
                    : <div className="absolute inset-0 flex items-center justify-center"><ImageIcon className="w-8 h-8 text-muted-foreground/40" /></div>}
                  {regen.isPending && <div className="absolute inset-0 bg-black/50 flex items-center justify-center"><Loader2 className="w-7 h-7 text-white animate-spin" /></div>}
                </div>
                <Button variant="outline" className="w-full gap-2 mt-2 text-xs" disabled={regen.isPending} onClick={() => regen.mutate({ id: editC.id })}>
                  <Sparkles className="w-3.5 h-3.5" /> Regerar imagem (15 créditos)
                </Button>
                <p className="text-[10px] text-muted-foreground mt-1">Mantém o estilo da marca e a direção de arte do post.</p>
              </div>

              {/* Briefing + copy editável */}
              <div className="space-y-3">
                <div className="flex flex-wrap gap-1.5">
                  {editMeta.pilar && <span className="text-[10px] font-bold text-primary bg-primary/10 rounded px-2 py-0.5">{editMeta.pilar}</span>}
                  {editMeta.formato && <span className="text-[10px] font-bold text-muted-foreground bg-muted rounded px-2 py-0.5 uppercase">{editMeta.formato}</span>}
                  {editMeta.angulo && <span className="text-[10px] font-bold text-muted-foreground bg-muted rounded px-2 py-0.5">{editMeta.angulo}</span>}
                </div>
                {editMeta.gancho && (
                  <div>
                    <Label className="text-[10px] text-muted-foreground uppercase font-black">Gancho (3s)</Label>
                    <p className="text-sm font-bold text-foreground">{editMeta.gancho}</p>
                  </div>
                )}
                <div>
                  <Label className="text-[10px] text-muted-foreground uppercase font-black">Legenda / Copy</Label>
                  <Textarea value={editCopy} onChange={(e) => setEditCopy(e.target.value)} className="bg-background border-border resize-none h-28 text-sm mt-1" />
                </div>
                {Array.isArray(editMeta.hashtags) && editMeta.hashtags.length > 0 && (
                  <p className="text-[11px] text-primary font-semibold">{editMeta.hashtags.map((h: string) => (h.startsWith("#") ? h : "#" + h)).join(" ")}</p>
                )}
                {editMeta.cta && <p className="text-xs"><span className="font-black text-muted-foreground">CTA:</span> {editMeta.cta}</p>}
                {editMeta.roteiro && (
                  <div className="rounded-lg border border-border p-2.5 bg-background">
                    <Label className="text-[10px] text-muted-foreground uppercase font-black">Roteiro de vídeo</Label>
                    <p className="text-[11px] font-bold text-foreground mt-1">🎬 {editMeta.roteiro.gancho3s}</p>
                    <div className="mt-1.5 space-y-1">
                      {(editMeta.roteiro.cenas ?? []).map((c: any, i: number) => (
                        <div key={i} className="text-[11px] text-muted-foreground flex gap-2">
                          <span className="font-black text-foreground whitespace-nowrap">{c.tempo}</span>
                          <span>{c.acao} <span className="opacity-60">· {c.audio}</span></span>
                        </div>
                      ))}
                    </div>
                    {editMeta.roteiro.cta && <p className="text-[11px] mt-1"><span className="font-black">CTA:</span> {editMeta.roteiro.cta}</p>}
                  </div>
                )}
                <div className="flex gap-2 pt-1">
                  <Button className="flex-1 gap-2 text-xs" disabled={saveCopy.isPending} onClick={() => saveCopy.mutate({ id: editC.id, copy: editCopy })}>
                    {saveCopy.isPending ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : null} Salvar
                  </Button>
                  <Button variant="outline" className="flex-1 gap-2 text-xs" disabled={sendApproval.isPending} onClick={() => sendApproval.mutate({ creativeIds: [editC.id] })}>
        ...
        <DialogContent className="max-w-sm bg-card border-border">
          <DialogHeader>
            <DialogTitle className="text-sm font-semibold flex items-center gap-2">
              <AlertTriangle className="w-4 h-4 text-destructive" /> Excluir criativo
            </DialogTitle>
          </DialogHeader>
          <p className="text-sm text-muted-foreground py-2">
            Tem certeza que deseja excluir <span className="font-bold text-foreground">{confirmDelete?.preview}</span>? Esta ação não pode ser desfeita.
          </p>
          <DialogFooter>
            ...
            <Button
              ...
              onClick={() => {
                ...
                deleteCreative.mutate({ id: confirmDelete.id }, { onSuccess: () => setConfirmDelete(null) });
              }}
            >
